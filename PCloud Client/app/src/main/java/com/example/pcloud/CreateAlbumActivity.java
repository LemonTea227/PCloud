    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        createAlbumButton.setOnClickListener(v -> {
            ...
            if (!albumNameCreateAlbum.getText().toString().matches("[\\u0001-\\u007e]+")) {
                albumNameCreateAlbumLayout.setError(getResources().getString(R.string.album_name_invalid_format));
            } else {
...
//                Toast.makeText(getApplicationContext(), "sucsses", Toast.LENGTH_SHORT).show();
                new Thread(new SendMessagesThread("NEW_ALBUM", MessageCodes.getRequest(),  albumNameCreateAlbum.getText().toString())).start();
            }

        });
        albumNameCreateAlbum.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
    ...
    public void messageReceived(String mes, Activity activity) {
        ...
        if (message.getName().equals("NEW_ALBUM")) {
            if (message.getType().equals(MessageCodes.getConfirm())) {
                ...
                Toast.makeText(getApplicationContext(), getResources().getString(R.string.new_album_error), Toast.LENGTH_SHORT).show();
            }
