    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginButtonLogin.setOnClickListener(v -> {
            ...
            if (!usernameLogin.getText().toString().matches("[a-zA-Z0-9]+")) {
                usernameLoginLayout.setError(getResources().getString(R.string.username_invalid_format));
            } else {
                usernameLoginLayout.setError(null);
            }
            if (passwordLogin.getText().toString().length() < 8 || !passwordLogin.getText().toString().matches("[\\u0001-\\u007e]+")) {
                passwordLoginLayout.setError(getResources().getString(R.string.password_invalid_format));
            } else {
...
//                Toast.makeText(getApplicationContext(), "sucsses", Toast.LENGTH_SHORT).show();
                new Thread(new SendMessagesThread("LOGIN", MessageCodes.getRequest(), String.format("%s\n%s", usernameLogin.getText().toString(), passwordLogin.getText().toString()))).start();
            }
        ...
        registerButtonLogin.setOnClickListener(v -> {
