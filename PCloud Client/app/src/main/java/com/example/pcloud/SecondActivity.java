...
public class SecondActivity extends AppCompatActivity implements ReceiveMessagesListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra("album_name")) {
            ...
            new Thread(new SendMessagesThread("PHOTOS", MessageCodes.getRequest(), getIntent().getExtras().getString("album_name"))).start();
        }

        if (getIntent().hasExtra("album_name") && getIntent().hasExtra("albums"))
        {
            adapter = new PhotosAdapter(getApplicationContext(),this.photos, getIntent().getExtras().getString("albums"), getIntent().getExtras().getString("album_name"));
        }
        else{
    ...
    private void generatePhotos(String message) {
        if (!message.equals("") && message != null) {
            ...
            for (int i = 0; i < decodedPhotos.length - 4; i += 4) {
                photos.add(new PhotosItem(photosNames[i], decodedPhotos[i], photosNames[i + 1], decodedPhotos[i + 1], photosNames[i + 2], decodedPhotos[i + 2], photosNames[i + 3], decodedPhotos[i + 3]));
                adapter.notifyItemInserted(0);
            }
            if (decodedPhotos.length % 4 == 1) {
                photos.add(new PhotosItem(photosNames[decodedPhotos.length - 1], decodedPhotos[decodedPhotos.length - 1]));
                adapter.notifyItemInserted(0);
            } else if (decodedPhotos.length % 4 == 2) {
                photos.add(new PhotosItem(photosNames[decodedPhotos.length - 2], decodedPhotos[decodedPhotos.length - 2], photosNames[decodedPhotos.length - 1], decodedPhotos[decodedPhotos.length - 1]));
                adapter.notifyItemInserted(0);
            } else if (decodedPhotos.length % 4 == 3) {
                photos.add(new PhotosItem(photosNames[decodedPhotos.length - 3], decodedPhotos[decodedPhotos.length - 3], photosNames[decodedPhotos.length - 2], decodedPhotos[decodedPhotos.length - 2], photosNames[decodedPhotos.length - 1], decodedPhotos[decodedPhotos.length - 1]));
                adapter.notifyItemInserted(0);
            } else if (decodedPhotos.length % 4 == 0 && decodedPhotos.length >= 4) {
                photos.add(new PhotosItem(photosNames[decodedPhotos.length - 4], decodedPhotos[decodedPhotos.length - 4], photosNames[decodedPhotos.length - 3], decodedPhotos[decodedPhotos.length - 3], photosNames[decodedPhotos.length - 2], decodedPhotos[decodedPhotos.length - 2], photosNames[decodedPhotos.length - 1], decodedPhotos[decodedPhotos.length - 1]));
                adapter.notifyItemInserted(0);
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_LOAD_IMAGE && resultCode == RESULT_OK && data != null) {
            ...
            String encodedImage = android.util.Base64.encodeToString(byteArrayOutputStream.toByteArray(), android.util.Base64.NO_WRAP);

            if (getIntent().hasExtra("album_name")) {
                new Thread(new SendMessagesThread("UPLOAD_PHOTO", MessageCodes.getRequest(), getIntent().getExtras().getString("album_name") + "\n" + nameOfFile + "\n" + encodedImage)).start();
            }
        }
    }
    ...
    public void messageReceived(String mes, Activity activity) {
        ...
        if (message.getName().equals("UPLOAD_PHOTO")) {
            if (message.getType().equals(MessageCodes.getConfirm())) {
                ...
                Toast.makeText(getApplicationContext(), getResources().getString(R.string.upload_photo_error), Toast.LENGTH_SHORT).show();
            }
        }
