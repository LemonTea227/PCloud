...
public class RegisterActivity extends AppCompatActivity implements ReceiveMessagesListener {
    TextInputLayout usernameRegisterLayout, passwordRegisterLayout, confirmPasswordRegisterLayout, firstNameRegisterLayout, lastNameRegisterLayout, birthDateRegisterLayout;
    TextInputEditText usernameRegister, passwordRegister, confirmPasswordRegister, firstNameRegister, lastNameRegister, birthDateRegister;
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                picker[0] = new DatePickerDialog(RegisterActivity.this,
                        new DatePickerDialog.OnDateSetListener() {
                            ...
                            public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                                birthDateRegister.setText(dayOfMonth + "/" + (monthOfYear + 1) + "/" + year);
                            }
                        }, year, month, day);
                picker[0].show();
            }
        ...
        registerButtonRegister.setOnClickListener(v -> {
            try {
            ...
            if (!usernameRegister.getText().toString().matches("[a-zA-Z0-9]+")) {
                usernameRegisterLayout.setError(getResources().getString(R.string.username_invalid_format));
            } else {
                usernameRegisterLayout.setError(null);
            }
            if (passwordRegister.getText().toString().length() < 8 || !passwordRegister.getText().toString().matches("[\\u0001-\\u007e]+")) {
                passwordRegisterLayout.setError(getResources().getString(R.string.password_invalid_format));
            } else {
                passwordRegisterLayout.setError(null);
            }
            if (confirmPasswordRegister.getText().toString().length() < 8 || !confirmPasswordRegister.getText().toString().matches("[\\u0001-\\u007e]+")) {
                confirmPasswordRegisterLayout.setError(getResources().getString(R.string.password_invalid_format));
            } else {
                confirmPasswordRegisterLayout.setError(null);
            }
            if (!firstNameRegister.getText().toString().matches("[a-zA-Z]+")) {
                firstNameRegisterLayout.setError(getResources().getString(R.string.name_invalid_format));
            } else {
            ...
            if (!birthDateRegister.getText().toString().matches("[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}")) {
                birthDateRegisterLayout.setError(getResources().getString(R.string.birt_date_invalid_format));
            } else {
                birthDateRegisterLayout.setError(null);
            }
            if (usernameRegisterLayout.getError() == null && passwordRegisterLayout.getError() == null && confirmPasswordRegisterLayout.getError() == null && firstNameRegisterLayout.getError() == null && lastNameRegisterLayout.getError() == null && birthDateRegisterLayout.getError() == null) {
//                Toast.makeText(getApplicationContext(), "sucsses", Toast.LENGTH_SHORT).show();
                new Thread(new SendMessagesThread("REGISTER", MessageCodes.getRequest(), String.format("%s\n%s\n%s\n%s", usernameRegister.getText().toString(), passwordRegister.getText().toString(), firstNameRegister.getText().toString() + " " + lastNameRegister.getText().toString(), birthDateRegister.getText().toString()))).start();
            }

        });

        usernameRegister.setOnFocusChangeListener((v, hasFocus) -> {
            usernameRegisterLayout.setError(null);
        });

        passwordRegister.setOnFocusChangeListener((v, hasFocus) -> {
            passwordRegisterLayout.setError(null);
        });

        confirmPasswordRegister.setOnFocusChangeListener((v, hasFocus) -> {
            confirmPasswordRegisterLayout.setError(null);
        });


        firstNameRegister.setOnFocusChangeListener((v, hasFocus) -> {
            firstNameRegisterLayout.setError(null);
        });


        lastNameRegister.setOnFocusChangeListener((v, hasFocus) -> {
            lastNameRegisterLayout.setError(null);
        });

        birthDateRegister.setOnFocusChangeListener((v, hasFocus) -> {
            if (hasFocus) {
        ...
        confirmPasswordRegister.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!confirmPasswordRegister.getText().toString().equals(passwordRegister.getText().toString())) {
                    confirmPasswordRegisterLayout.setError(getResources().getString(R.string.confirm_password_invalid_format));
                } else {
                    confirmPasswordRegisterLayout.setError(null);
                }
            }
    ...
    public void onBackPressed() {
        if (usernameRegister.getText().toString().equals("") && passwordRegister.getText().toString().equals("") && confirmPasswordRegister.getText().toString().equals("") && firstNameRegister.getText().toString().equals("") && lastNameRegister.getText().toString().equals("") && birthDateRegister.getText().toString().equals("")) {
    ...
    public void messageReceived(String mes, Activity activity) {
        ...
        if (message.getName().equals("REGISTER")) {
            if (message.getType().equals(MessageCodes.getConfirm())) {
                ...
                Toast.makeText(getApplicationContext(), getResources().getString(R.string.register_error), Toast.LENGTH_SHORT).show();
            }
        }
        else if(message.getName().equals("ALBUMS")) {
            if (message.getType().equals(MessageCodes.getConfirm())) {
                Toast.makeText(getApplicationContext(), getResources().getString(R.string.getting_in), Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(getApplicationContext(), getResources().getString(R.string.albums_error), Toast.LENGTH_SHORT).show();
            }
        }
    }
}